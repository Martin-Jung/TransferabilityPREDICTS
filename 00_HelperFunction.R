...
#### Do a time series outlier removal and smooth ### 

preparePixel <- function(val,years){
  val <- as.ts(zoo(val,years))
  # identify outliers
  try(outl <- forecast::tsoutliers(val),silent = TRUE)
  if(!exists("outl")) {return(val)}
  # Replace outliers
  val[outl$index]<- outl$replacements
